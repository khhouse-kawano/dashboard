...
const DatabaseResale = ({ onReload, key }: Props) => {
    ...
    const handleCopy = (id: string) => {
        if (!id) return;
        const customerName = originalDatabase.find(o => o.id === id)?.customer ?? '';
        if (window.confirm(`${customerName}様のデータを複製しますか?`)) {
            const postData = {
                id,
                newId: generateULID(),
                request: 'database_resale_copy'
            };
            ...
            const fetchData = async () => {
                try {
                    ...
                    console.error(error);
                    alert(`エラー: ${error.response?.data?.message || "通信エラーが発生しました"}`);
                }
            };
            fetchData();
        };
