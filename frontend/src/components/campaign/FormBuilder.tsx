...
import { BUILDER_FIELDS, BuilderField, buildHtml } from './formBuilderUtils';
import { CAMPAIGN_BRANDS } from './brands';
...
const FormBuilder: React.FC<Props> = ({ activeTab }) => {
    ...
    useEffect(() => {
        if (!brand || !campaignId) { setFields(BUILDER_FIELDS); return; }
    ...
    /** 使う項目。⚠️ 既定では必須のものだけを選んだ状態にする */
    const [used, setUsed] = useState<Record<string, boolean>>(
        () => Object.fromEntries(BUILDER_FIELDS.map(f => [f.key, f.required]))
    );
            ...
            <div className="bg-white" style={{ width: '90%', maxWidth: '960px', margin: '0 auto', padding: '24px', paddingBottom: '120px' }}>
                ...
                <div className="mb-3">
                    ...
                    <select value={brand} onChange={e => { setBrand(e.target.value); setCampaignId(''); setHtml(''); }}
                        ...
                        {CAMPAIGN_BRANDS.map(b => <option key={b} value={b}>{b}</option>)}
                    </select>
                </div>
                ...
                {/* ---- 項目 ---- */}
                <div className="mb-4">
                    <div style={{ fontSize: '13px', marginBottom: '6px' }}>聞き取る項目</div>
