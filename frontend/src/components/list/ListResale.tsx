...
const ListResale = ({ onReload }: Props) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                await setStaffList(response.data.staff.filter(s => s.period === String(thisYear) && s.section === '中古住宅専門店'));
    ...
    const handleSync = async (idValue: string) => {
        ...
        if (!idValue || !filteredCustomer || !categoryValue) {
            alert(`同期に失敗しました。${!categoryValue ? ' ※カテゴリーが未選択' : ''}`);
            return;
        }
        ...
        if (window.confirm(`${categoryValue} ${filteredCustomer.first_name} ${filteredCustomer.last_name}様 顧客情報を取り込みますか?`)) {
            const postData = {
                ...
                in_charge_user: filteredCustomer.staff ? filteredCustomer.staff : '中古住宅専門店 店舗管理', //担当営業
    ...
    const inquiryFilter = (shopValue: string) => {
        return inquiryList.filter(c => (shopValue ? c.category === shopValue : true) && selectedMonth.includes(monthFormate(c.inquiry_date))).length;
    };
    ...
    const reserveFilter = (shopValue: string) => {
        return customerList.filter(c => (shopValue ? c.shop === shopValue : true) && selectedMonth.includes(monthFormate(c.reserve))).length;
    };
        ...
        <>  {isSyncing && <div style={{ position: 'absolute', top: '30vh', width: '60vw', left: 'calc( 50% - 30vw)', zIndex: '2000', height: '220px', backgroundColor: 'white', boxShadow: '0px 5px 15px 0px rgba(0, 0, 0, 0.35)', padding: '80px 100px 100px' }}>
            ...
            <div className='inquiry_table spec bg-white p-2'>
                <div className="d-flex flex-wrap mb-3 align-items-center" style={{ paddingTop: isSp ? '30px' : '' }}>
                    ...
                    <div className="m-1">
                        <select className="target" onChange={(e) => setTargetCategory(e.target.value)} style={{ fontSize: '13px', }}>
                                            ...
                                            <>{item.black_list.split('support').length % 2 === 0 || item.black_list.split('black').length % 2 === 0 || item.shop.includes('重複') ? <i className="fa-solid fa-xmark"></i> :
                                                item.sync === 1 ? <span style={{ textDecoration: 'none', backgroundColor: 'blue', padding: '3px 7px', color: '#fff', borderRadius: '3px', cursor: 'pointer' }}
                                                    ...
                                                    <i className='fa-solid fa-arrows-rotate sticky-column'
                                                        style={{ opacity: item.category ? '1' : '.3', cursor: item.category ? 'pointer' : '' }}
                                                        onClick={() => item.category ? handleSync(item.inquiry_id) : null}
                                                    ></i>
                                            }</>
