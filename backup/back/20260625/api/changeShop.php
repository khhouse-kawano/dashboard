...
try {
    ...
    if ($demand === 'shop' && $shop !== 'duplicate') {
        ...
        if (!$customers) {
            $insertSql = 'INSERT INTO customers (id, name, register, shop, reserved_status, response_status, campaign, zip)
            VALUES (:id, :name, :register, :shop, :reserved_status, :response_status, :campaign, :zip)';
            $stmtInsertSql = $pdo->prepare(($insertSql));
            $stmtInsertSql->bindValue(':id', $data['id'], PDO::PARAM_STR);
            $stmtInsertSql->bindValue(':name', $data['name'], PDO::PARAM_STR);
            ...
            $stmtInsertSql->bindValue(':campaign', $data['campaign'], PDO::PARAM_STR);
            $stmtInsertSql->bindValue(':zip', $data['zip'], PDO::PARAM_STR);
        ...
        $masterSql = 'SELECT * FROM master_data WHERE id = ?';
        $masterStmt = $pdo->prepare(($masterSql));
        $masterStmt->execute([$data['id']]);
        $masterCustomer = $masterStmt->fetch(PDO::FETCH_ASSOC);

        if (!$masterCustomer) {
            $insertMasterSql = 'INSERT INTO master_data (id, customer_contacts_name, customer_contacts_name_kana, full_address, customer_contacts_mobile_phone_number, customer_contacts_email, postal_code)
            VALUES (:id, :customer_contacts_name, :customer_contacts_name_kana, :full_address, :customer_contacts_mobile_phone_number, :customer_contacts_email, :postal_code)';
            $stmtInsertMasterSql = $pdo->prepare(($insertMasterSql));
            $stmtInsertMasterSql->bindValue(':id', $data['id'], PDO::PARAM_STR);
            $stmtInsertMasterSql->bindValue(':customer_contacts_name', $data['customer_contacts_name'], PDO::PARAM_STR);
            $stmtInsertMasterSql->bindValue(':customer_contacts_name_kana', $data['customer_contacts_name_kana'], PDO::PARAM_STR);
            $stmtInsertMasterSql->bindValue(':full_address', $data['full_address'], PDO::PARAM_STR);
            $stmtInsertMasterSql->bindValue(':customer_contacts_mobile_phone_number', $data['customer_contacts_mobile_phone_number'], PDO::PARAM_STR);
            $stmtInsertMasterSql->bindValue(':customer_contacts_email', $data['customer_contacts_email'], PDO::PARAM_STR);
            $stmtInsertMasterSql->bindValue(':postal_code', $data['postal_code'], PDO::PARAM_STR);
            if ($stmtInsertMasterSql->execute()) {
                echo json_encode(['status' => 'success']);
            } else {
                echo json_encode(['status' => 'error']);
            }
