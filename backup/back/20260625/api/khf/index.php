...
if ($demand === 'resale') {
    ...
    if ($result) {
        ...
        $sql = "INSERT INTO resale_customers (
          staff, registered, reserved, contract, rank, id_related, medium, response, flag
        ) VALUES (
          :staff, :registered, :reserved,  :contract, :rank, :id_related, :medium, :response,'exist'
        )";
...
} elseif ($demand === 'medium_list') {
    $pdo = new PDO($dsn, $db_user, $db_password);
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = 'SELECT * FROM medium_list';
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    echo json_encode($result ? $result : '無', JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
