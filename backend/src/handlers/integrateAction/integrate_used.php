...
if (isset($data['request']) && $data['request'] === 'integrate') {
    ...
    try {
        ...
        // 1. ベース（統合先）の現在データを取得
        $stmt = $pdo->prepare("SELECT * FROM master_data_resale WHERE id = :id FOR UPDATE");
        ...
        if (!empty($cleanIntegrateIds)) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM master_data_resale WHERE id IN ($placeholders) FOR UPDATE");
        ...
        if (!empty($cleanIntegrateIds)) {
            $placeholders = implode(',', array_fill(0, count($cleanIntegrateIds), '?'));
            $stmt = $pdo->prepare("UPDATE master_data_resale SET show_dashboard = '0', integration = '0' WHERE id IN ($placeholders)");
            $stmt->execute($cleanIntegrateIds);
        }
