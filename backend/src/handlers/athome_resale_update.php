...
try {
    ...
    if ($checkEmail !== '') {
        $checkStmt = $pdo->prepare("SELECT id FROM athome_db_resale WHERE email = :email LIMIT 1");
    ...
    if ($existingRecord) {
        $sql = "UPDATE athome_db_resale SET 
        ...
        if ($stmt->execute($params)) {
            require_once __DIR__ . '/portal/athome_resale.php';
        ...
        $sql = "INSERT INTO athome_db_resale 
