...
if (!$id) {
    die(json_encode(['status' => 'error', 'message' => 'IDが指定されていません']));
}

try {
    // 1. メインの物件情報を取得 (※テーブル名は環境に合わせてください)
    $stmt = $pdo->prepare("SELECT * FROM `property_db` WHERE `property_id` = ?");
    $stmt->execute([$id]);
    $property = $stmt->fetch(PDO::FETCH_ASSOC);

    // 2. SUUMO情報を取得
    $stmt = $pdo->prepare("SELECT * FROM `suumo_resale_summary` WHERE `management_no` = ?");
    $stmt->execute([$id]);
    $suumo = $stmt->fetch(PDO::FETCH_ASSOC);

    // 3. HOME'S情報を取得
    $stmt = $pdo->prepare("SELECT * FROM `homes_summary_db` WHERE `company_management_number` = ?");
    $stmt->execute([$id]);
    $homes = $stmt->fetch(PDO::FETCH_ASSOC);

    // 4. athome情報を取得 (完全一致 or 下6桁が一致するものをLIKE検索)
    $stmt = $pdo->prepare("SELECT * FROM `athome_summary_db` WHERE `management_no` = ? OR `management_no` LIKE ?");
    $stmt->execute([$id, "%" . $id]);
    $athome = $stmt->fetch(PDO::FETCH_ASSOC);

    // 5. 顧客情報を取得 (※本来はここでLIKE検索等で絞り込むのが綺麗ですが、フロントの normalizePropertyName 互換性を保つため、顧客一覧を返します)
