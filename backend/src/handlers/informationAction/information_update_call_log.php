...
try {
    ...
    if ($stmt->rowCount() > 0) {
        $updateSql = 'UPDATE call_sheet SET call_log = :call_log, status = :status, shop = :shop, staff = :staff, reserved_status = :reserved_status WHERE id = :id';
        ...
        $updateStmt->bindValue(':staff', $data['staff'] ?? '', PDO::PARAM_STR);
        ...
        if ($updateStmt->execute()) {
            $response = ['status' => 'success', 'message' => $data['name'] . '様の架電記録の登録に成功しました。'];
            echo json_encode($response, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        } else {
            $response = ['status' => 'error', 'message' => $data['name'] . '様の架電記録の登録に失敗しました。'];
            echo json_encode($response, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
    } else {
        $insertSql = 'INSERT INTO call_sheet (id, shop, name, staff, call_log, status, reserved_status) VALUES (:id, :shop, :name, :staff, :call_log, :status, :reserved_status)';
        $insertStmt = $pdo->prepare($insertSql);
        $insertStmt->bindValue(':id', $data['id'], PDO::PARAM_STR);
        $insertStmt->bindValue(':shop', $data['shop'] ?? '', PDO::PARAM_STR);
        $insertStmt->bindValue(':staff', $data['staff'] ?? '', PDO::PARAM_STR);
        ...
        if ($insertStmt->execute()) {
            $response = ['status' => 'success', 'message' => $data['name'] . '様の架電記録のアップデートに成功しました。'];
            echo json_encode($response, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        } else {
            $response = ['status' => 'error', 'message' => $data['name'] . '様の架電記録のアップデートに失敗しました。'];
            echo json_encode($response, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
    }
