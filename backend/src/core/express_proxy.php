...
function expressProxyRequests(): array
{
    return [
        ...
        // -----------------------------------------------------------------
        // 2026-09-16 移植。他社広告ライブラリ（meta_ads）。
        //
        // ⚠️⚠️ **request 名だけで書いてはいけない。**
        //   ⚠️ 移植元の PHP（backend/src/handlers/meta_ads.php）は
        //     **1つの request で読み書きを兼ねていた**（`id` があれば更新）。
        //   ⚠️ request だけで許可すると、書き込みも ② へ流れる。
        //
        // ⚠️ PHP ハンドラは**残してある**。この2行を消せば即座に ① へ戻る。
        //   ⚠️ そのため `bookmark` は expressProxyExclusive() にも入れている
        //     （転送失敗で ① にも流れると、二重に更新される経路ができる）。
        // -----------------------------------------------------------------
        'meta_ads:list',
        'meta_ads:bookmark',

        // -----------------------------------------------------------------
        // 2026-09-16 移植。失注一覧（参照のみ）。
        //
        // ⚠️ ① に PHP ハンドラが実在する（lostList.php）。この行を消せば即座に戻る。
        ...
        // ⚠️⚠️ **アップロード（competitor_pdf_upload）は ① に残す。**
        //   ⚠️ ファイルの実体は ① の uploads/competitors/ にあり、
        //     ⚠️ **② から ① のファイルシステムへは書けない。**
        //   ⚠️ multipart は shouldProxyToExpress() が転送しないが、
        //     ⚠️ **ここに書かないこと自体が防御**である。
        // -----------------------------------------------------------------
        'competitor_pdf',
        ...
        //
        // ⚠️⚠️ **① に PHP ハンドラは無い。最初から Express だけにある。**
        //   ⚠️ ⚠️ **expressProxyExclusive() にも入れてある**（分析レポートと同じ理由）。
        // -----------------------------------------------------------------
        'satbase_list',
        'satbase_update',
        ...
        //
        // ⚠️ ① に PHP ハンドラが実在する（campaignSummary.php）。
        //   この行を消せば即座に ① へ戻る。
        // ⚠️ 書き込みは無いので expressProxyExclusive() へは入れない。
        // -----------------------------------------------------------------
        'campaignSummary',
        ...
        // -----------------------------------------------------------------
        // 2026-09-14 移植。広告費シミュレーター（header/BudgetSimulator.tsx）。
        //
        // ⚠️ 参照のみ。① に budget_simulator.php が実在するのでフォールバックしてよい。
        //
        // ⚠️⚠️ **注文と建売をまとめて返すため応答が大きい。**
        //   転送に時間がかかる場合は forwardToExpress の CURLOPT_TIMEOUT（120秒）
        //   に近づく。2026-09-11 に別の request が 17MB・120秒で切れている。
        //   ⚠️ 列を足すときは転送量を意識すること。
        // -----------------------------------------------------------------
        'budget_simulator',

        // -----------------------------------------------------------------
        // 2026-09-14 移植。顧客一覧（database/DatabaseOrder.tsx / DatabaseKaeru.tsx）。
        //
        // ⚠️ 参照のみ。① に database.php が実在するのでフォールバックしてよい。
        //
        // ⚠️⚠️ **category を必ず書くこと。** request 名だけで書くと、
        //   ② に登録していない 'used' / 'common' まで ② へ送られ、
        //   「ループ検知」で 502 になる。
        //
        // ⚠️⚠️ **roll 付き（trash / copy）は登録しない。** 書き込みを伴う別経路で、
        //   ② には移植していない。`database::order` の空の roll がその意味である。
        //
        // ⚠️⚠️ **応答が大きい。** 顧客は注文で約24,000件・建売で約10,000件。
        //   転送に時間がかかる場合は forwardToExpress の CURLOPT_TIMEOUT（120秒）
        //   に近づく。⚠️ 列を足すときは転送量を意識すること。
        // -----------------------------------------------------------------
        'database::order',
        'database::spec',
        ...
        // -----------------------------------------------------------------
        // 2026-09-15 新規。Google クチコミの取得（projects/sync から呼ばれる）。
        //
        // ⚠️⚠️ **① に PHP ハンドラは無い。** 以前の post_review.php /
        //   shop_review.php は現行の handlers/ から消えており、
        //   backup/back/20260625/api/actions/ にだけ残っている。
        //   ⚠️ そのため `save` は expressProxyExclusive() にも入れている。
        //     転送に失敗したら 502 で終わる（① には処理が無いので当然だが、
        //     明示しておかないと将来 PHP を作ったときに二重実行になる）。
        //
        // ⚠️ roll ごとに登録すること。`google_review` だけで書くと、
        //   未登録の roll が ② へ送られて「ループ検知」で 502 になる。
        // -----------------------------------------------------------------
        'google_review:list',
        'google_review:save',
...
function expressProxyExclusive(): array
{
    return [
        // -----------------------------------------------------------------
        // 2026-09-16 移植。他社広告ライブラリのブックマーク。
        //
        // ⚠️ ① に PHP ハンドラが**実在する**（meta_ads.php）。
        //   ⚠️ 転送に失敗して ① にも流れると、同じ更新が2回走る。
        //   ⚠️ 冪等な更新（0/1 の上書き）なので実害は小さいが、
        //     **防御に頼る構成にしない**ため入れている。
        // -----------------------------------------------------------------
        'meta_ads:bookmark',
        ...
        // 2026-09-21 追加。分析レポート（Claude が書いた HTML）。
        //
        // ⚠️⚠️ **① に PHP ハンドラが存在しない。** ⚠️ 落ちても ① では処理できない。
        //   ⚠️ ここに入れておくと ⚠️ **502 を返す**ので、利用者にエラーが見える。
        //   ⚠️ ⚠️ **入れないと ① が404を返し、「該当する処理がありません」になる。**
        // -----------------------------------------------------------------
        'analysis_report_list',
        'analysis_report_get',
        'analysis_report_upload',
        ...
        // -----------------------------------------------------------------
        // 2026-09-15 新規。Google クチコミの保存（projects/sync から呼ばれる）。
        //
        // ⚠️⚠️ ① に PHP ハンドラは**無い**（post_review.php は現行の
        //   handlers/ から消えている）。転送に失敗すれば 502 で終わる。
        //   ⚠️ ここに書いておくのは、**将来 ① に PHP を作ったときに
        //     二重実行にならないようにするため**である。
        //   ⚠️ クチコミの追記は「持っていないものだけ足す」形なので
        //     二重に走っても増えはしないが、review_history には
        //     同じ日の点が2つ入りうる。
        // -----------------------------------------------------------------
        'google_review:save',
