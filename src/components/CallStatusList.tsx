...
const CallStatus = ({ callStatusShow, setCallStatusShow, shopArray, monthArray, staffArray, originalDatabase, insideSalesCategory, setInsideSalesCategory }: Props) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.post('https://khg-marketing.info/dashboard/api/gateway/', { request: 'callStatusList' }, { headers });
    ...
    const targetShops = shopArray.filter(s => s.section === '熊本営業課').map(s => s.shop);
                                ...
                                <Table bordered striped>
                                    <tbody style={{ fontSize: '11px' }}>
                                        ...
                                        {[{ brand: '', shop: '熊本営業課', section: '熊本営業課' }, ...shopArray].sort().filter(s => s.section === '熊本営業課').map((s, sIndex) => {
                                            ...
                                            const registerFilter = originalDatabase.filter(o => sIndex === 0 ? targetShops.includes(o.shop) : o.shop === s.shop);
                                            ...
                                            return ['総反響数', '対応反響数', '対応中', 'アポ取得数', '対応反響数からの来場数', '総架電数', '資料郵送数', 'SMS送信数', 'メール送信数'].map((item, index) => <tr>
                                                ...
                                                {['total', ...monthArray.slice(8)].map((month, mIndex) => {
                                                    ...
                                                    const formattedCallFilter = callFilter.filter(call => mIndex === 0 ? true : dateFormate(call.day).includes(month));
                                                    const formattedPostFilter = postFilter.filter(call => mIndex === 0 ? true : dateFormate(call.day).includes(month));
                                                    const formattedSmsFilter = smsFilter.filter(call => mIndex === 0 ? true : dateFormate(call.day).includes(month));
                                                    const formattedContinueFilter = continueFilter.filter(c => {
                                                        ...
                                                        return mIndex === 0 ? true : dateFormate(newest.day).includes(month)
                                                    });
                                                    const formattedAppointFilter = appointFilter.filter(interview => mIndex === 0 ? true : dateFormate(interview.reserved_status).includes(month));
                                                    const formattedMailFilter = mailFilter.filter(call => mIndex === 0 ? true : dateFormate(call.day).includes(month));
                                                    const formattedInterviewFilter = interviewFilter.filter(interview => mIndex === 0 ? true : dateFormate(interview.reserved_status).includes(month));
                                                    ...
                                                    return <td style={{ textAlign: 'right' }} className={`${mIndex === 0 ? 'fw-bold ' : ''}${index === 4 ? 'fw-bold text-primary table-primary' : index === 3 ? 'fw-bold text-danger table-danger' : (index === 2 || index === 1) ? 'fw-bold' : ''}`}>
                                                        ...
                                                        {index === 1 && formattedCalledCustomer.length}
                                                        {index === 2 && formattedContinueFilter.length}
                                                        {index === 3 && `${formattedAppointFilter.length + formattedInterviewFilter.length}(${perAppoint}%)`}
                                                        {index === 4 && formattedInterviewFilter.length}
