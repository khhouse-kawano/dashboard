...
interface ShopItem { shop: string; }
...
const getPhase = (item: CustomerItem): PhaseType => {
    ...
    if (isTruthy(item.screening) || isTruthy(item.interview) || isTruthy(item.contract)) return "interview";
    return "lead";
};
...
const Map: React.FC = () => {
    ...
    useEffect(() => {
        if (!brand?.trim() || !token?.trim() || !category?.trim()) {
            navigate("/login");
            return;
        }
        if (!isLoaded) return;
        ...
        const fetchData = async () => {
            try {
                ...
                const response = await axios.post("https://khg-marketing.info/dashboard/api/gateway/", { request: "map" }, { headers });
            ...
            <Card className="shadow-sm mb-4 border-0">
                <Card.Body>
                    <Row className="g-2">
                        ...
                        <Col xs={12} sm={6} md={2}>
                            <Form.Select size="sm" value={targetBrand} onChange={(e) => {
                        ...
                        <Col xs={12} sm={6} md={2}>
                            <Form.Select size="sm" value={targetShop} onChange={(e) => {
                                ...
                                {shop
                                    .filter((item) => !item.shop.includes("未設定") && !item.shop.includes("全店舗"))
                                    .map((item, index) => (
                                        <option key={`shop-${index}`} value={item.shop}>{item.shop}</option>
                                    ))}
                            </Form.Select>
                        </Col>
                        <Col xs={12} sm={6} md={2}>
