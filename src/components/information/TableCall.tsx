...
const TableCall = ({ information, setInformation, callLog, setCallLog, interviewer, call, setCall }: Props) => {
    ...
    const newCall = () => (
        <div className="d-flex align-items-center" style={{ fontSize: '11px', fontWeight: '500', marginBottom: '4px', letterSpacing: '.6px', verticalAlign: 'middle' }}>
            <div>
                <input type="date" style={{ ...inputStyle, width: '120px' }} value={call.day}
            ...
            <div>
                <input type="time" step="60" style={{ ...inputStyle, width: '120px' }} value={call.time}
                    onChange={(e) => setCall(prev => ({
                        ...prev,
            ...
            <div>
                <input type="text" style={{ ...inputStyle, width: '120px' }} value={userName}
                    disabled />
            </div>
            ...
            <div>
                <textarea value={call.note} style={{ ...inputStyle, width: '300px' }} placeholder='アクション内容・ヒアリング内容を記載'
                    onChange={(e) => setCall(prev => ({
                        ...prev,
                ...
                {callLog.call_log &&
                    callLog.call_log
                        ...
                        .map((item, index) => <>
                            ...
                            <div className="d-flex align-items-center" style={{ fontSize: '11px', fontWeight: '500', marginBottom: '4px', letterSpacing: '.6px', verticalAlign: 'middle' }}>
                                <div>
                                    <input type="date" value={item.day} style={{ ...inputStyle, width: '120px' }}
                                        onChange={(e) => setCallLog(prev => ({
                                            ...prev,
                                ...
                                <div>
                                    <input type="time" value={item.time} style={{ ...inputStyle, width: '120px' }}
                                        onChange={(e) => setCallLog(prev => ({
                                            ...prev,
                                            call_log: prev.call_log.map((log, i) => i === index ?
                                ...
                                <div>
                                    <input type="text" value={item.staff ?? ''} style={{ ...inputStyle, width: '120px' }}
                                        disabled />
                                </div>
                                <div style={{ fontSize: '11px', fontWeight: '500', letterSpacing: '.6px', verticalAlign: 'middle', marginLeft: '5px' }}>
                                    <select style={{ ...inputStyle, width: '140px' }} value={item.action}
                                        onChange={(e) => setCallLog(prev => ({
                                            ...prev,
                                            call_log: prev.call_log.map((log, i) => i === index ?
                                ...
                                <div>
                                    <textarea style={{ ...inputStyle, width: '300px', height: 'auto' }} placeholder='アクション内容・ヒアリング内容を記載' value={item.note} rows={Math.max(2, item.note.length / 50)}
                                        onChange={(e) => setCallLog(prev => ({
                                            ...prev,
                                            call_log: prev.call_log.map((log, i) => i === index ?
