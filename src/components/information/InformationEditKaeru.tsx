...
import { valueStyle } from '../../utils/informationUtils';
import { inputStyle } from '../../utils/informationUtils';
import { selectStyle } from '../../utils/informationUtils';
import { requiredStyle } from '../../utils/informationUtils';
import { safeFormate } from '../../utils/informationUtils';
import { competitorsStyle } from '../../utils/informationUtils';
...
const InformationEditKaeru = ({ id, token, onClose, authority }: Props) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                setOriginalPropertyList(response.data.property.filter(p => p.store_name === '株式会社　国分ハウジング不動産　本店').map(p => p.property_name));
    ...
    const handleSave = async () => {
        const requiredList = ['customer_contacts_name', 'in_charge_store', 'in_charge_user'];
                                    ...
                                    <tr>
                                        ...
                                        <td style={valueStyle}>
                                            <div className="">
                                                ...
                                                />
                                                <TableInput information={information} setInformation={setInformation} itemKey='planned_construction_site_2' defaultValue='入居希望地②'
                                                />
                                                <TableInput information={information} setInformation={setInformation} itemKey='planned_construction_site_3' defaultValue='入居希望地③'
                                                />
                                            </div>
                                        </td>
