...
const CustomerTrendKaeru: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const responseCustomer = response.data.customer.map(r => (!r.medium || !mediums.includes(r.medium)) ? ({
          ...r,
          medium: 'その他'
        }) : r);
  ...
  useEffect(() => {
    ...
    const filteredCustomer = originalUserData.filter(o =>
      ...
      (portalChecked ? !isHp(o.hp_campaign) : true)
    );
  ...
  const isHp = (value: string) => {
    ...
    return campaign.includes(value);
  };
                  ...
                  {['全販促媒体', 'ホームページ反響計', ...mediumArray]
                    .map((medium, mediumIndex) => {
                      ...
                      const base = userData.filter(o =>
                        mediumIndex === 0 ? true :
                          mediumIndex === 1 ? isHp(o.hp_campaign) : formate(o.medium) === formate(medium));
                              ...
                              <td style={{ fontSize: '11px', letterSpacing: '.5px' }}>
                                <div className={checked.register.show ? "text-white p-2 rounded" : 'text-white rounded'} style={{ backgroundColor: '#6baed6' }}>
                                  ...
                                  <div className={checked.contact.show ? "rounded px-2 py-1 my-2" : "my-2 rounded"} style={{ backgroundColor: '#4292c6' }}>
                                    {checked.contact.show && <div>接触:<span className="fw-bold">{contact.length.toLocaleString()}</span>({isNaN(contact.length / total.length) ? 0 : Math.floor(contact.length / total.length * 100)}%)
                                    </div>}
                                    <div className={(checked.interview.show || checked.tour.show) ? "rounded px-2 py-1 my-2" : "my-2 rounded"} style={{ backgroundColor: '#08519c' }}>
                                      {checked.interview.show && <div className='my-1'>店舗来場:<span className="fw-bold">{interview.length.toLocaleString()}</span>({isNaN(interview.length / contact.length) ? 0 : Math.floor(interview.length / contact.length * 100)}%)
                                      </div>}
                                      {checked.tour.show && <div className='my-1'>物件案内:<span className="fw-bold">{tour.length.toLocaleString()}</span>({isNaN(tour.length / contact.length) ? 0 : Math.floor(tour.length / contact.length * 100)}%)
                                      </div>}
