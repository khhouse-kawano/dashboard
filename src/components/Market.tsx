...
type ResponseData = { period: string, orderRegister: number, orderReserve: number, orderContract: number, specRegister: number, specReserve: number, specContract: number };
...
type Contract = { constructionDate: string, category: string, address: string, pref: string, shop: string };
type ConstructionKhf = { name: string, address: string, category: string, constructionDate: string, completionDate: string, finishedDate: string };
...
const Resale = () => {
    ...
    const [khgConstruction, setKhfConstruction] = useState<ConstructionKhf[]>([]);
    const [originalKhfConstruction, setOriginalKhfConstruction] = useState<ConstructionKhf[]>([]);
    ...
    const generationMapping: (keyof Population)[] = [
        'age_0_4',
        'age_5_9',
        'age_10_14',
        'age_15_19',
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const [populationResponse, householdsResponse, customerResponse, buildResponse, customerResponse_khf, shopResponse, staffResponse, mediumResponse, contractResponse, constructionKhfResponse] = await Promise.all([
                ...
                axios.post("https://khg-marketing.info/dashboard/api/", { demand: "medium_marketing" }, { headers }),
                axios.post("https://khg-marketing.info/dashboard/api/", { demand: "contract_list" }, { headers }),
                axios.post("https://khg-marketing.info/dashboard/api/", { demand: "construction_list_khf" }, { headers })
            ]);
            ...
            const mediumArray = mediumResponse.data.filter((item: { ma_medium: number; ma_category: string; }) => item.ma_medium === 1 && item.ma_category !== '').map((item: { ma_medium: number; ma_category: string; }) => item.ma_category) as string[];
            setMedium([...new Set(mediumArray)]);
            setMediumCategory(mediumResponse.data);
            setOriginalContract(contractResponse.data);
            setOriginalKhfConstruction(constructionKhfResponse.data);
        };
        fetchData();
    }, []);

    useEffect(() => {
        const monthList = getYearMonthArray(2025, 1);
        const startIndex = startMonth ? monthList.indexOf(startMonth) : 0;
        const endIndex = endMonth ? monthList.indexOf(endMonth) : monthList.length;
        const filteredMonth = monthList.slice(startIndex, endIndex + 1);
        setTargetMonth(filteredMonth);
        ...
        const filteredCustomer = originalCustomerList.filter(item => {
            ...
            return (targetShop.shop ? item.shop === targetShop.shop : true) &&
                (targetMedium ? medium.includes(item.medium) : true)
        });

        const condominiumsStaff = staff.filter(item => item.shop === targetShop.shop).map(item => item.name);
        const filteredKhfCustomer = originalKhfCustomerList.filter(item => {
            const medium = mediumCategory.filter(m => m.ma_category === targetMedium).map(m => m.medium);
            return (targetShop.shop ? condominiumsStaff.includes(item.staff) : true) &&
                (targetMedium ? medium.includes(item.medium) : true)
        });
        ...
        const filteredConstruction = originalContract.filter(o => {
            const construction = new Date(o.constructionDate);
            return construction >= startDate && construction <= endDate &&
                (targetPref !== '' ? o.pref === targetPref : true) &&
                (targetShop.shop !== '' ? o.shop === targetShop?.shop.replace('店', '').replace('霧島', '本社') : true)
        });

        setContract(filteredConstruction);

        const filteredKhfConstruction = originalKhfConstruction.filter(o => filteredMonth.includes(o.constructionDate?.replace(/-/g, '/').slice(0, 7)));

        setKhfConstruction(filteredKhfConstruction)

    }, [originalPopulation, originalCustomerList, originalKhfCustomerList, originalContract, originalKhfConstruction, targetPref, targetGender, startAge, endAge, targetArea, originalBuild, startMonth, endMonth, targetShop, targetMedium]);


    const mediumExpand = async (index: number, area: string) => {
        setModalTitle(area === '-' ? `${targetPref}全域` : area);
        ...
        if (area === '-') {
            ...
            filteredBuildLength = originalBuild.filter(b => b.area === area).map(b => ({
                period: b.year,
                order: b.owner,
        ...
        setBuildLineData(filteredBuildLength.sort((a, b) => new Date(a.period).getTime() - new Date(b.period).getTime()));
        setResponseLineData(filteredResponseLength.sort((a, b) => new Date(a.period).getTime() - new Date(b.period).getTime()));
        const filteredPopulationLength: AgeData[] = generationMapping.map(g => {
            const targetAmount = originalPopulation.find(p => p.pref === targetPref && p.area === area && p.gender === "計")?.[g];
            const targetMale = originalPopulation.find(p => p.pref === targetPref && p.area === area && p.gender === "男")?.[g];
            ...
            return {
                ...
                male: Number(targetMale ?? 0),
                female: Number(targetFemale ?? 0),
            };
        });
        setPopulationLineData(filteredPopulationLength);
        ...
        setHouseholdsLineData(filteredHouseholdsLength);
    ...
    useEffect(() => {
        const orderSummary = monthArray.map(month => {
            function formatValue(date: string) {
                return date.replace(/-/g, '/');
            }
            const areaValue = modalTitle.includes('全域') ? targetPref : modalTitle;
            const registerValue = customerList.filter(c => formatValue(c.register).includes(month) && c.full_address.includes(areaValue)).length;
            const reserveValue = customerList.filter(c => formatValue(c.reserve).includes(month) && c.full_address.includes(areaValue)).length;
            const contractValue = customerList.filter(c => formatValue(c.contract).includes(month) && c.full_address.includes(areaValue)).length;
            const constructionValue = contract.filter(c => c.category === '注文' && (modalTitle?.includes('全域') ? c.pref === targetPref : c.address?.includes(modalTitle)) && formatValue(c.constructionDate).includes(month)).length;
            const areaConstructionValue = modalTitle.includes('全域') ? build.filter(b => b.pref === targetPref && b.year === month).reduce((acc, cur) => acc + cur.owner, 0) : build.filter(b => b.area === modalTitle && b.year === month).reduce((acc, cur) => acc + cur.owner, 0);
            return {
                title: '期間ごとの対象商圏における反響数',
                area: areaValue,
                period: month,
                category: '注文住宅営業',
                ...
                areaConstruction: areaConstructionValue,
                share: `${Math.ceil(constructionValue / areaConstructionValue * 1000) / 10}%`
            }
        });

        const specSummary = monthArray.map(month => {
            function formatValue(date: string) {
                return date.replace(/-/g, '/').slice(0, 7);
            }
            const areaValue = modalTitle.includes('全域') ? targetPref : modalTitle;
            const registerValue = khfCustomerList.filter(c => month === formatValue(c.registered) && (modalTitle.includes('全域') ? c.address.trim().includes(targetPref) : c.address.trim().includes(areaValue))).length;
            const reserveValue = khfCustomerList.filter(c => month === formatValue(c.reserved) && (modalTitle.includes('全域') ? c.address.trim().includes(targetPref) : c.address.trim().includes(areaValue)) && c.reserved).length;
            const contractValue = khfCustomerList.filter(c => month === formatValue(c.contract) && (modalTitle.includes('全域') ? c.address.trim().includes(targetPref) : c.address.trim().includes(areaValue)) && c.contract).length;
            const pref = targetShop.shop?.replace('かえる', '').replace('店', '県');
            let khfAreas: string[] = [pref];
            if (pref === '鹿児島県') {
                khfAreas.push('都城');
            }
            const constructionValue = khgConstruction.filter(c => (modalTitle.includes('全域') ? c.address.includes(targetPref) : c.address.includes(modalTitle)) &&
                (c.address.includes(pref) || (pref === '鹿児島県' ? c.address.includes('都城') : pref === '宮崎県' ? !c.address.includes('都城') : true)) && formatValue(c.constructionDate) === month
            ).length;

            const areaConstructionValue = modalTitle.includes('全域') ? build.filter(b => b.pref === targetPref && b.year === month).reduce((acc, cur) => acc + cur.condominiums, 0) : build.filter(b => b.area === areaValue && b.year === month).reduce((acc, cur) => acc + cur.condominiums, 0);
            return {
                title: '期間ごとの対象商圏における反響数',
                area: areaValue,
                period: month,
        ...
        const populationSummary = generationMapping.map(g => {
            const areaValue = modalTitle.includes('全域') ? '-' : modalTitle;
            const targetAmount = originalPopulation.find(p => p.pref === targetPref && p.area === areaValue && p.gender === "計")?.[g];
            const targetMale = originalPopulation.find(p => p.pref === targetPref && p.area === areaValue && p.gender === "男")?.[g];
            const targetFemale = originalPopulation.find(p => p.pref === targetPref && p.area === areaValue && p.gender === "女")?.[g];
            return {
                title: '世代ごとの対象商圏における男女別人口',
                area: modalTitle.includes('全域') ? targetPref : modalTitle,
                age: g.replace('age_', '').replace('_', '~'),
                amount: targetAmount,
                male: targetMale,
                female: targetFemale,
        ...
        const houseType = ['総数', '一戸建', '賃貸']
        let filteredHouseholds: HouseholdsData[];
        if (modalTitle.includes('全域')) {
            filteredHouseholds = houseType.map(h => {
                const onePersonUnder30Length = originalHouseholds.filter(o => o.pref === targetPref && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.one_person_under30, 0);
                const onePerson30_64Length = originalHouseholds.filter(o => o.pref === targetPref && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.one_person_30_64, 0);
                const onePersonOver65Length = originalHouseholds.filter(o => o.pref === targetPref && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.one_person_over65, 0);
                const wifeHusbandLength = originalHouseholds.filter(o => o.pref === targetPref && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband, 0);
            ...
            filteredHouseholds = houseType.map(h => {
                const onePersonUnder30Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.one_person_under30, 0);
                const onePerson30_64Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.one_person_30_64, 0);
                const onePersonOver65Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.one_person_over65, 0);
                const wifeHusbandLength = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband, 0);
                const wifeHusbandOver65Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_over65, 0);
                const wifeHusbandChildUnder3Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_child_under3, 0);
                const wifeHusbandChild3_5Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_child_3_5, 0);
                const wifeHusbandChild6_9Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_child_6_9, 0);
                const wifeHusbandChild10_17Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_child_10_17, 0);
                const wifeHusbandChild18_24Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_child_18_24, 0);
                const wifeHusbandChildOver25Length = originalHouseholds.filter(o => o.area === modalTitle && (h === '賃貸' ? (o.type === '長屋建' || o.type === '共同住宅') : o.type === h)).reduce((acc, cur) => acc + cur.wife_husband_child_over25, 0);
                return (
                    {
                        title: '住宅種類ごとの対象商圏における世帯数内訳',
                        area: modalTitle.includes('全域') ? targetPref : modalTitle,
        ...
        const mediumSummary = medium.map(item => {
            function formatValue(date: string) {
                return date.replace(/-/g, '/').slice(0, 7);
            }
            const areaValue = modalTitle.includes('全域') ? targetPref : modalTitle;
            const originalMonthArray = getYearMonthArray(2025, 1);
            ...
            const targetMonth = originalMonthArray.slice(startIndex, endIndex);
            const mediumList = mediumCategory.filter(m => m.ma_category === item).map(m => m.medium);
            const registerValue = customerList.filter(c => mediumList.includes(c.medium) && c.full_address.includes(areaValue) && targetMonth.includes(formatValue(c.register))).length;
            const reserveValue = customerList.filter(c => mediumList.includes(c.medium) && c.full_address.includes(areaValue) && targetMonth.includes(formatValue(c.reserve))).length;
            const contractValue = customerList.filter(c => mediumList.includes(c.medium) && c.full_address.includes(areaValue) && targetMonth.includes(formatValue(c.contract))).length;
            return {
                title: '販促媒体ごとの対象商圏における反響数内訳',
                area: modalTitle.includes('全域') ? targetPref : modalTitle,
                category: '注文住宅営業',
                medium: item,
                register: registerValue,
        ...
        const mediumKhfSummary = medium.map(item => {
            function formatValue(date: string) {
                return date.replace(/-/g, '/').slice(0, 7);
            }
            const areaValue = modalTitle.includes('全域') ? targetPref : modalTitle;
            ...
            const targetMonth = originalMonthArray.slice(startIndex, endIndex);
            const mediumList = mediumCategory.filter(m => m.ma_category === item).map(m => m.medium);
            const registerValue = khfCustomerList.filter(c => mediumList.includes(c.medium) && c.address.includes(areaValue) && targetMonth.includes(formatValue(c.registered))).length;
            const reserveValue = khfCustomerList.filter(c => mediumList.includes(c.medium) && c.address.includes(areaValue) && targetMonth.includes(formatValue(c.reserved))).length;
            const contractValue = khfCustomerList.filter(c => mediumList.includes(c.medium) && c.address.includes(areaValue) && targetMonth.includes(formatValue(c.contract))).length;
            return {
                title: '販促媒体ごとの対象商圏における反響数内訳',
                area: modalTitle.includes('全域') ? targetPref : modalTitle,
                category: '建売住宅営業',
        ...
        const data = [...orderSummary, ...specSummary, ...populationSummary, ...mediumSummary, ...mediumKhfSummary, ...filteredHouseholds];
        const fetchData = async () => {
            try {
                const headers = { Authorization: '4081Kokubu', 'Content-Type': 'application/json' };
                const response = await axios.post("https://sync-pg-cloud-9f739ab131ed.herokuapp.com/api/areasummary", { data }, { headers });
                // const response = await axios.post("http://localhost:3000/api/areasummary", { data }, { headers });
    ...
    return (
        <>
            ...
            <Modal show={show} onHide={modalClose} size='xl'>
                ...
                <Modal.Body>
                    {geminiApi.show ? <div>{gemini ?
                        <>
                            <div className="text-center my-3">
                                <div className="rounded-pill mt-2 aiButton" style={{ fontSize: '11px', cursor: 'pointer' }}
                                    onClick={() => {
                                        setGemini('');
                                        setGeminiApi(prev => ({ area: prev.area, show: false }));
                                        setGeminiApi({ area: modalTitle, show: true });
                                    }}>AIによる分析開始</div>
                            </div>
                        ...
                        : <div className="text-center mt-1 mb-5"><div className="rounded-pill mt-2 aiButton" style={{ fontSize: '11px', cursor: 'pointer' }}
                            onClick={() => setGeminiApi({ area: modalTitle, show: true })}>AIによる分析開始</div></div>}
                    <div className="mb-5">
                        ...
                        <div style={{ width: "100%", height: '300px' }}>
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    <Legend
                                        ...
                                        content={({ payload }) => (
                                            <div className='d-flex justify-content-center'>
                                                {["orderRegister", "orderReserve", "orderContract"].map(key => {
                                                    const entry = payload?.find(p => p.dataKey === key);
                                                    return (
                                                        <div className='m-1 px-2 py-1 text-white rounded' key={key} style={{ backgroundColor: entry?.color }}>
                                                            {entry?.value}
                                                        </div>
                                                    );
                                                })}
                                    ...
                                    />
                                    <Line type="monotone" dataKey="orderRegister" stroke="#dc3545" strokeWidth={2} name='注文総反響' />
                                    <Line type="monotone" dataKey="orderReserve" stroke="#fd7e14" strokeWidth={2} name='注文来場' />
                                    <Line type="monotone" dataKey="orderContract" stroke="#6f42c1" strokeWidth={2} name='注文契約' />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    <Legend
                                        ...
                                        content={({ payload }) => (
                                            <div className='d-flex justify-content-center'>
                                                {["specRegister", "specReserve", "specContract"].map(key => {
                                                    const entry = payload?.find(p => p.dataKey === key);
                                                    return (
                                                        <div className='m-1 px-2 py-1 text-white rounded' key={key} style={{ backgroundColor: entry?.color }}>
                                                            {entry?.value}
                                    ...
                                    />
                                    <Line type="monotone" dataKey="specRegister" stroke="#ff6f61" strokeWidth={2} name='建売総反響' />
                                    <Line type="monotone" dataKey="specReserve" stroke="#ffb347" strokeWidth={2} name='建売来場' />
                                    <Line type="monotone" dataKey="specContract" stroke="#a569bd" strokeWidth={2} name='建売契約' />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                        ...
                        <div style={{ width: "100%", height: '300px' }}>
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    />
                                    <Line type="monotone" dataKey="order" stroke="#0d6efd" strokeWidth={2} name='注文住宅着工棟数' />
                                    <Line type="monotone" dataKey="condo" stroke="#198754" strokeWidth={2} name='分譲住宅着工棟数' />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
