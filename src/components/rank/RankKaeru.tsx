...
const RankKaeru = () => {
    ...
    const setNewRank = async (idValue: string, newRank: string, periodValue: string) => {
        ...
        const postData = {
            ...
            request: 'rank_kaeru_update_rank'
        };
        try {
    ...
    const closeInformationEdit = async () => {
        try {
            const response = await axios.post('https://khg-marketing.info/dashboard/api/gateway/', { request: "rank_kaeru_customer_info" }, { headers });
            console.log(response.data.status);
            await setCustomerList(response.data.newCustomers);
        } catch (e) {
            console.error(e);
                ...
                <div className="">
                    <Table bordered>
                        <tbody style={{ fontSize: '12px' }} className='align-middle'>
                            ...
                            {[{ label: '建売分譲事業', category: 'all', show: true }, ...labelList].map((target, targetIndex) => {
                                let bgKey;
