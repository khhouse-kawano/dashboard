...
const PortalBuy = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                setMediumArray(mediumResponse.data.filter(m => m.resale_portal_medium === 1)); //初期フィルター
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const filteredReserved = originalCustomers.filter(o => {
            return o.flag === 'exist' && (targetMedium ? o.medium === targetMedium : mediumArray.map(m => m.medium).includes(o.medium)) && monthArray.includes(o.reserved.slice(0, 7).replace('-', '/'))
        });
        setReservedCustomers(filteredReserved);

        const filteredContract = originalCustomers.filter(o => {
            return o.flag === 'exist' && (targetMedium ? o.medium === targetMedium : mediumArray.map(m => m.medium).includes(o.medium)) && monthArray.includes(o.contract.slice(0, 7).replace('-', '/'))
        });
