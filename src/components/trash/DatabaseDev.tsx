...
type customerList = { id: string; shop: string; name: string; staff: string; status: string; rank: string; medium: string; reserve: string; register: string; before_survey: number; before_interview: number; after_interview: number; call_status: string, reserved_status: string, full_address: string; phone_number: string; trash: number };
...
const Database = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const headers = { Authorization: '4081Kokubu', 'Content-Type': 'application/json' };
      const masterDataRes = await axios.post("https://khg-marketing.info/survey/api/", data, { headers });
      console.log(masterDataRes.data);
    } catch (error) {
      console.error("データ取得エラー:", error);
    }
            ...
            <Table responsive style={{ fontSize: '11px', textAlign: 'center' }} bordered striped className='list_table database_list'>
              ...
              <tbody>
                {filteredDatabase.slice(sliceStart, sliceStart + basicLength).map((item, index) =>
                  <tr key={index}>
                    <td><div className='hover bg-danger text-white' style={{ fontSize: "12px", cursor: 'pointer', width: 'fit-content', padding: '4px 10px', borderRadius: '5px', margin: '0 auto', textDecoration: 'none' }} onClick={() => showModal(item.id, 'information_edit', '', '')}>編集</div></td>
                    <td>{item.shop}</td>
                    <td>{item.name}</td>
                    <td>{item.staff}</td>
                    <td>{item.status}</td>
                    ...
                    <tr id='inquiry_reason' className={selected.inquiry_reason ? 'table-secondary' : undefined} onClick={() => handleSetSelected('inquiry_reason', 'body')}>
                      ...
                      <td style={{ fontSize: '13px', letterSpacing: '.6px' }}>
                        {inquiryReasons.map((item, index) =>
                          <div className="form-check" style={{ fontSize: '13px', letterSpacing: '.5px' }} key={index}>
