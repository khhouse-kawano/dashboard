...
const CustomersDev = () => {
    ...
    const [datasetsRegisterArray, setDatasetsRegisterArray] = useState<number[]>([]);
    const [datasetsReserveArray, setDatasetsReserveArray] = useState<number[]>([]);
    const [datasetsContractArray, setDatasetsContractArray] = useState<number[]>([]);
    ...
    useEffect(() => {
        ...
        setDatasetsRegisterArray(registerLengthArray);
        ...
        setDatasetsReserveArray(reserveLengthArray);

        const contractLengthArray =  mediumArray.map( mediumItem => {
            return filtered.filter( filteredItem => filteredItem.medium === mediumItem.medium && filteredItem.reserve !== '').length
        });
        setDatasetsContractArray(contractLengthArray);

        setColorArray(mediumArray.map( (_, index) => colorCodes[index]));
        ...
        <div className='container bg-white py-3 mt-2'>
            <div>
                ...
                <div className="d-flex py-3">
                    <select className="form-select campaign position-relative" onChange={(event) =>handleSort(event.target.value, endMonth, selectedShop, selectedSection, selectedArea)}>
                    ...
                    <select className="form-select campaign position-relative me-2" onChange={(event) =>handleSort(startMonth, event.target.value, selectedShop, selectedSection, selectedArea)}>
                        <option value="" selected>終了月</option>
                        {monthArray.map((month, index) => (<option key={index} value={month}>{month}</option>
                            ))}
                    </select>
                    <select className="form-select campaign position-relative me-2" onChange={(event) =>handleSort(startMonth, endMonth, event.target.value, '', '')}>
            ...
            <Table striped style={{ fontSize: '12px'}}>
                ...
                <tbody>
                    ...
                    .map((value, index) => {
                        ...
                        return(
                        <tr key={index}>
                            <td style={{textAlign: 'center'}}>{value.medium}</td>
