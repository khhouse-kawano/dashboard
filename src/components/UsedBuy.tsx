...
const UsedBuy = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                setOriginalCustomers(customerResponse.data.filter(c => c.case.slice(0, 1) === '買')); //初期フィルター
                setStaff(staffResponse.data);
                setAchievement(achievementResponse.data);
                setMediumArray(mediumResponse.data.filter(m => m.resale_medium === 1));//初期フィルター
            } catch (error) {
    ...
    useEffect(() => {
        const filteredRegistered = originalCustomers.filter(o => {
            const isExist = o.flag === 'exist';
            const isTargetMedium = targetMedium ? o.medium === targetMedium
                : true;
            const isTargetMonth = monthArray.includes(o.registered.replace(/-/g, '/').slice(0, 7));

            return isExist && isTargetMedium && isTargetMonth;
        });
        ...
        const filteredReserved = originalCustomers.filter(o => {
            ...
            const isTargetMedium = targetMedium ? o.medium === targetMedium
                : true;
            const isTargetMonth = monthArray.includes(o.reserved.replace(/-/g, '/').slice(0, 7));
            return isExist && isTargetMedium && isTargetMonth;
        });
        ...
        const filteredContract = originalCustomers.filter(o => {
            ...
            const isTargetMedium = targetMedium ? o.medium === targetMedium
                : true;
            const isTargetMonth = monthArray.includes(o.contract.replace(/-/g, '/').slice(0, 7));
            return isExist && isTargetMedium && isTargetMonth;
        });
                                        ...
                                        <div style={{ minWidth: `${(monthArray.length + 1) * 60 + 200}px`, fontSize: '12px' }}>
                                            <Table bordered striped style={{ tableLayout: 'fixed', width: 'auto' }}>
                                                <tbody className='align-middle'>
                                                    ...
                                                    {[{ id: 0, medium: '全媒体', resale_medium: 1 }, ...mediumArray, { id: mediumArray.length + 1, medium: '不明', resale_medium: 1 }]
                                                        .sort((a, b) => {
                                                            const countA =
                                                                sortKey === 'registered'
