...
const CustomerTrend: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.post("https://khg-marketing.info/dashboard/api/gateway/", { request: 'customerTrend' }, { headers });
    ...
    const getValue = (base: CustomerList[], monthIndex: number, month: string, target: string, period: string[] = monthArray) => {
        ...
        if (target === 'contract') {
            return base.filter(b => (monthIndex >= 1 ? dateFormate(b.contract).includes(month) && b.status === '契約済み' : period.includes(dateFormate(b.contract).slice(0, 7)) && b.status === '契約済み'))
        }
