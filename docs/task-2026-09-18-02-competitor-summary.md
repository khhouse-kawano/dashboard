...
| モーダル | タイトル |
|---|---|
| 失注（失注列） | ⚠️ **`{競合会社名} 失注一覧`**（指示どおり） |
| ⚠️ 契約（契約列） | ⚠️ **`{競合会社名} に勝った案件`**（⚠️ **変更の指示が無いため従来のまま**） |

⚠️ ⚠️ **並びとしては不揃いである。** ⚠️ 揃える場合は `{競合会社名} 契約一覧` などが考えられるが、
⚠️ **勝手に変えていない。** ⚠️ 指示があれば1行で直せる（`openCards` に渡す文字列だけ）。
