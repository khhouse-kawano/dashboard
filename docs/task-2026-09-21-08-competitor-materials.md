...
| ⚠️ **`company` を埋められた** | ⚠️ **42 / 53** |

⚠️⚠️ **`company` は「その顧客の `competitors_text` に載っている名前」しか入れていない。**
⚠️ 画面の選択タグはそこから作られるため、⚠️ **載っていない名前を入れると選び直せなくなる。**
⚠️ ⚠️ **42件すべて一致することを機械的に検証した。**

⚠️ 空にした11件の理由:

| 理由 | 件数 |
|---|---|
| ⚠️ PDF に社名の表記が無い（平面図・パース） | 5 |
| ⚠️ 台帳の競合候補に無い名前（森建築 / タマホーム シフクノ家 / 七呂仕様書） | 3 |
| ⚠️ 1ファイルに2社分が混在 | 2 |
| ⚠️ **自社のDM** | 1 |
...
| ⚠️ 分類の適用 | ⚠️ **category 53件 / company 42件** |
| ⚠️ `company` が台帳の候補と一致するか | ⚠️ **42件すべて一致**（機械的に検証） |
