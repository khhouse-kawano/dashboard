...
### ⚠️ 1. スクロールできなかった理由（⚠️ **flex**）

⚠️⚠️ **`App.tsx` はメニューと本文を `d-flex` で並べている**（95行目）。
⚠️ ⚠️ **flex の子は既定（`min-width: auto`）で、中身より小さくならない。**
⚠️ ⚠️ **そのため囲みが表の幅まで広がり、`overflow` が一度も効かなかった。**

```css
.rk_page { flex: 1 1 auto; min-width: 0; width: 100%; }
```

⚠️ ⚠️ **`min-width: 0` を付けて初めて中の `overflow` が働く。**
⚠️ ⚠️ **`<Routes>` は DOM を作らないので、各画面の一番外側の div がそのまま flex の子になる。**

### ⚠️ 2. バーが見えなかった理由（⚠️ **高さ**）

⚠️ 横スクロールバーは ⚠️ **スクロールする箱のいちばん下**に付く。
⚠️ ⚠️ **行が多いと箱が縦に長くなり、バーが画面の外に出て見えない。**

```css
.rk_scroll_x { max-height: 78vh; }
```

⚠️ ⚠️ **高さを画面内に収めると、横のバーも画面内に入る。**

⚠️ 併せて ⚠️ **バーそのものを見えるようにした**（既定では触るまで出ない環境がある）。

```css
.rk_scroll_x { scrollbar-width: auto; scrollbar-color: #94a3b8 #eef2f7; }
.rk_scroll_x::-webkit-scrollbar { width: 12px; height: 12px; }
.rk_scroll_x::-webkit-scrollbar-thumb { background: #94a3b8; border-radius: 999px; border: 3px solid #eef2f7; }
```

⚠️⚠️ **横に隠れている列があると気づけないのが、いちばん困る。**

### ⚠️ 未確認

⚠️⚠️ **この環境にヘッドレスブラウザが無く、実際の描画では確かめられていない。**
⚠️ ⚠️ **ブラウザでの確認をお願いしたい**（下のチェックリスト）。

⚠️ 2段見出しは ⚠️ **固定していない**（`rowSpan` と相性が悪く、崩れる危険があるため）。
⚠️ ⚠️ **縦に長い表では、下までスクロールすると見出しが見えなくなる。**
⚠️ **必要なら次の版で対応する。**
