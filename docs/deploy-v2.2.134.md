...
⚠️⚠️ **v2.2.132 / v2.2.133 も一緒に出る。** `origin/production` は **v2.2.131** で止まっており、
`v2.2.134` ブランチには 132（キャンペーンフォームの Express 化）と
133（MetaAds・失注一覧・メール文面編集）も**まとめて入っている**。
⚠️ 途中のどれかだけを先に出すことはできない（同じ枝の上にある）。

⚠️ ⚠️ **`v2.2.133` の手順書は使わない。** ⚠️ こちらが上位互換である。
...
| 9 | 失注先「不明」ボタン | 顧客詳細 |
| 10 | ⚠️ **メールの差出人名がブランドごとに変わる** | （サーバー側） |
...
1. `git push origin v2.2.134`（ブランチまで）
2. GitHub で `v2.2.134` → `production` の PR を作る
3. PR をマージする

⚠️ `v2.2.132` / `v2.2.133` のブランチは push していない。⚠️ **不要**（134 に含まれている）。
...
⚠️ ⚠️ **このままでよい。** ⚠️ v2.2.134 からは**表示名だけ**をブランド名で
上書きするようになった。⚠️ **アドレスは `SMTP_FROM` のものを使い続ける。**

⚠️ ⚠️ **アドレスを変えないこと。** ⚠️ 送信元として認められていないアドレスにすると
**メールがまるごと弾かれる**。
...
⚠️ 画面右下（またはメニュー）のバージョンが **2.2.134** になっていること。
...
| ⚠️ **差出人名** | ⚠️ **そのブランドの名前**（⚠️ `国分ハウジング` で固定されていないこと） |

⚠️⚠️ **差出人名は v2.2.134 で変わるところ。** ⚠️ 今までは `SMTP_FROM` の
「国分ハウジング」が全ブランドで出ていた。⚠️ ここが直っているのが今回の肝。
...
| なごみ工務店 | なごみ工務店 |
| ニーエルホーム | ニーエルホーム |
| フルコミホーム | ⚠️ **フルコミコーム**（⚠️ 誤字のまま。下の宿題を見る） |
| ジャスフィーホーム | JUSFY HOME |
| PG HOUSE | PG-HOUSE |
| かえるホーム | かえるホーム |
| ⚠️ KHG 共通 | ⚠️ **選んだ店舗のブランド名**（`国分ハウジンググループ` ではない） |

⚠️ ⚠️ **メールアドレスは変わらない**（`SMTP_FROM` のまま）。
⚠️ アドレスまで変えると送信元として認められず**まるごと弾かれる**。
...
| 7 | ⚠️ **フルコミホームの表示名が `フルコミコーム`**（誤字） | 中 |

⚠️ 7 について: ⚠️ ① の PHP にも**同じ誤字**があり、⚠️ **今も本番でその名前で届いている**。
⚠️ ただし今までは全部「国分ハウジング」で隠れており、⚠️ **v2.2.134 から顧客に見えるようになる**。
⚠️ 直すなら `backend-express/src/features/campaignForm/brands.ts` の `name` 1か所
（⚠️ `value` と `shopPrefix` は触らないこと）。
