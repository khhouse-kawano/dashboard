...
| 宛先 | 件名 |
|---|---|
| フォームに入力されたアドレス | `【国分ハウジング】お問い合わせありがとうございます` |
| `mkt@kh-house.jp` | `【公式アンバサダーからの反響/<アンバサダー名>様】<顧客名>様` |

⚠️ **送信は保存を確定させたあとに行う。** 順序を入れ替えると、メールサーバーが
不調な間の反響が丸ごと失われる。

⚠️ **送信に失敗しても 200 を返す。** 500 にすると、保存済みなのに顧客が
「送信に失敗しました」を見て再送し、同じ反響が重複する。

結果は `mail_sent` / `notify_sent` に記録し、反響一覧に `顧✓ 社✓` で表示する。

### ⚠️⚠️ 送信元は khg-marketing.info（① レンタルサーバー）から

`kh-house.jp` から送ってはいけない。以下は**実際に起きた障害**である。

`kh-house.jp` の MX はさくら + Google Workspace を向いており、**Xserver では
受信していない。** それなのに Xserver 上に `noreply@kh-house.jp` を作ると、
Xserver は「`kh-house.jp` は自分のドメインだ」と判断し、`mkt@kh-house.jp` 宛を
**MXを見ずに内部配送する。** さくらにも Google にも届かず、エラーも出ない。

症状は「顧客宛（外部ドメイン）は届くのに、社内宛だけ届かない」。
ログには送信失敗が一切残らないため、原因にたどり着きにくい。

⚠️ **この判定は差出人ではなく宛先ドメインで決まる。** From を変えるだけでは
直らず、**送信サーバーごと ① に移す**必要がある。

加えて `kh-house.jp` の SPF に Xserver が含まれていない（`~all` なので
配送はされるが迷惑メール扱いになる）。`khg-marketing.info` の SPF は
`+a:sv13020.xserver.jp ... include:spf.sender.xserver.jp` を含むため、
① から送れば正規の送信元として認証される。

| サーバー | ドメイン | 用途 |
|---|---|---|
| `sv13020.xserver.jp` | `khg-marketing.info` | ① レンタルサーバー。**ここから送る** |
| `sv11135.xbiz.ne.jp` | `kh-house.jp` | Xserver ビジネス。⚠️ 送信に使わない |
