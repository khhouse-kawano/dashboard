...
| 10 | ⚠️ **失注の必須項目を「他決理由・敗因」の2つに絞る**（⚠️ 要回答が大きく減る） | ⚠️ **フロント・② ・①** |
| 11 | ⚠️ **建売のメニューに「店舗別広告費」を戻す** | フロント |
| 12 | ⚠️ **店舗別広告費（建売）を販促媒体別広告費に揃える**（⚠️ **数字が大きく変わる**） | フロント |
...
| ⚠️ `backend/src/handlers/menu.php` | `dashboard/api/gateway/handlers/menu.php` | ⚠️ **失注の要回答から2列を外す** |
...
| 2 | ⚠️ 価格差・今後の対策を空のまま保存 | ⚠️ **止まらない**（見出しは「任意」） |
| 3 | 「詳細な他決・失注理由」 | ⚠️ **必須の赤い印**（⚠️ 注文事業のみ） |
| 4 | ⚠️ 反響一覧の「要回答」 | ⚠️ **大きく減る**（ローカルでは 89件 → 4件） |
| 5 | ⚠️ メニューの失注バッジ | ⚠️ **反響一覧と同じ件数** |
| 6 | 失注リストの「未入力箇所」 | ⚠️ **価格差・今後の対策が出ない** |
| 7 | ⚠️ 建売のメニュー | ⚠️ **「店舗別広告費」が出る** |

### ⚠️⚠️ 7-0c. 建売の「店舗別広告費」（⚠️ **数字が大きく変わる**）

| # | 確認 | 期待 |
|---|---|---|
| 1 | 媒体で「Web検索」を選ぶ | ⚠️ **総反響が大きく増える**（ローカルでは 214件 → 5,129件） |
| 2 | 同上の総予算 | ⚠️ **全額ではなくなる**（ローカルでは ¥281,556,856 → ¥99,576,458） |
| 3 | 「全販促媒体」に戻す | ⚠️ **改修前と同じ数字** |
| 4 | ⚠️ **契約率** | ⚠️ **分母が「申込」に変わり大きくなる**（ローカルでは 10% → 57%） |
...
| ⚠️ 注文事業の「店舗別広告費」 | ⚠️ **何も変わっていない** |
| ⚠️ 建売の反響推移 | ⚠️ **何も変わっていない** |

⚠️ ⚠️ **建売の「店舗別広告費」は変わる。** ⚠️ 7-0c を参照。
⚠️ 契約率の分母が「申込」になり、⚠️ **販促媒体別広告費と同じ意味の列**になった。
...
| 9 | ⚠️ 建売の `ShopKaeru.tsx` に**来場率・申込率・来場単価も足すか**（⚠️ 今回は契約率のみ揃えた） | 中 |
