...
| 11 | ⚠️ フルコミホームの表示名の誤字（`フルコミコーム`）を修正 | （サーバー側） |
...
| なごみ工務店 | なごみ工務店 |
| ニーエルホーム | ニーエルホーム |
| フルコミホーム | ⚠️ **フルコミホーム**（⚠️ 移植元の誤字 `フルコミコーム` を直した） |
...
| 7 | ⚠️ ① の PHP に残る `フルコミコーム` の誤字 | 中 |

⚠️ 7 について: ⚠️ ② は直した。⚠️ ① の PHP は**まだ誤字のまま**なので、
⚠️ **② が落ちて ① へ退避したときだけ `フルコミコーム` で届く。**
⚠️ [khg-api-form-register.snippet.php](../backend/forms/khg-api-form-register.snippet.php) を
反映するときに一緒に直す。
