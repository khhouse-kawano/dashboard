...
## ⚠️ 8. 顧客分析と反響推移の「ホームページ反響」を揃える

⚠️ 指示:

> CustomerTrendKaeru.tsx の全期間列のホームページ反響と揃うようにしてほしい

### ⚠️ 測ったら原因は2つあった

| | 顧客分析 | 反響推移（全期間） |
|---|---|---|
| 揃える前 | ⚠️ **6,463** | ⚠️ **4,415** |

| 原因 | 件数 | 中身 |
|---|---|---|
| ⚠️⚠️ **対象期間** | ⚠️ **1,855** | ⚠️ 反響推移の「全期間」は `getYearMonthArray(2025, 1)` で ⚠️ **2025年1月以降のみ**。⚠️ 顧客分析はそれ以前と反響日なしも含んでいた（2,545件） |
| ⚠️ 判定の違い | ⚠️ **193** | ⚠️ Web検索・Instagram の無条件の丸め（顧客分析にだけ入れていた） |

⚠️ ⚠️ **期間が9割だった。** ⚠️ **判定だけ揃えても一致しない。**

### 決めたこと（確認のうえ）

⚠️ ⚠️ **反響推移も丸める**（利用者の選択）。⚠️ **両方 4,608件で一致する。**

| 画面 | 直した内容 |
|---|---|
| `CustomerTrendKaeru.tsx` | ⚠️ **`HOMEPAGE_MEDIUMS` の丸めを `isHpGroup` に足した**（⚠️ **+193件**） |
| `CustomerKaeru.tsx` | ⚠️ **期間の起点を `PERIOD_START = '2025/01'` にした**（顧客・販促費とも） |
...
⚠️⚠️ **判定は `customerKaeruUtils.ts` の1か所に置き、両画面が読んでいる。**
⚠️ ⚠️ **片方だけ直すと必ずまた食い違う。**

⚠️ ⚠️ **反響推移の数字は +193 変わる。** ⚠️ **利用者の判断である。**

---

## ⚠️ 9. 勝敗分析を担当者・店舗で絞れるようにする
...
> 先ほども伝えたが first_interviewed_user の項目及び interview_sheet テーブルも参照すること

### ⚠️ なぜできなかったか

⚠️⚠️ **スタッフ軸を足したのは集計の口（`pivot` / `funnel`）だけだった。**
⚠️ ⚠️ **勝敗分析は別の口（`/analysis/competitor`）で、絞り込みが `division` と `months` しか無かった。**

### 直した内容

| | |
|---|---|
| ② `/analysis/competitor` | ⚠️ **`staff` / `shop` の絞り込み**、行に ⚠️ **`staff` と `first_staff`** を追加 |
| MCP `get_competitor_deals` | ⚠️ **同じ2つの引数** |
...
| 絞り込み | 件数 |
|---|---|
| なし | 781（勝281 / 負500・⚠️ 上限に当たっている） |
| KH八代店 | 260（勝21 / 負239） |
| ⚠️ **KH八代店 × 井上 健太郎（現在の担当だけ）** | ⚠️ 12（勝10 / 負2） |
| ⚠️⚠️ **KH八代店 × 井上 健太郎（旧担当・面談シートも見る）** | ⚠️ **15（勝12 / 負3）** |

⚠️ ⚠️ **3件増えたぶんが「引き継いだ商談」と「本人が面談した商談」である。**

⚠️ meta には ⚠️ **「いま担当していない商談も本人の実績として出る」**と明記した。
⚠️ MCP の説明文には ⚠️ **「絞ると件数が一気に減る。数十件で勝率を語らないこと」**を入れた。

---

## ⚠️ 10. 分析APIを建売分譲事業に対応させる

⚠️ 指示:

> v2.2.145で建売部門の分析までできるように
> master_data_kaeru を使用
> KPIや歩留まりの設定は CustomerTrendKaeru.tsx ShopTrendKaeru.tsx を参照
> 可能な限り注文に揃えたい
...
| | 決定 |
|---|---|
| 指標名 | ⚠️ **共通名で揃える**（`visits` / `nextAppointments` / `contracts`） |
| 建売の失注 | ⚠️ **返さない**（`lost` は null） |
...
| 列 | 注文 | 建売 |
|---|---|---|
| ⚠️ **`01J82Z5F1RR18Z792C7KZS88QG`** | ⚠️ **契約** | ⚠️ **申込** |
| `01JP74NGRTT95X4Z8AQZ2QK2PW` | ⚠️ 使わない | ⚠️ **契約** |
| `01JV6AVXQMJY6XR4STWCHNKVE0` | ⚠️ 使わない | ⚠️ **仲介契約** |
| `01JV6AVXR4X6HW3JQ0G53Y26GG` | ⚠️ 使わない | ⚠️ **物件案内** |

⚠️ ⚠️ **注文の定義をそのまま当てると、申込を契約として数える。**

### 建売の工程（`ShopTrendKaeru.tsx` に合わせた）
...
| 工程 | 列 |
|---|---|
| 接触 | `01J82Z5F1990Y4G2TZ6XSCRX3Z`（⚠️ 注文の「通電」と同じ列） |
| ⚠️ **来場・案内** | `01J82Z5F1GQB…` ＋ ⚠️ **`01JV6AVXR4…`（物件案内）** |
| 次アポ | `01JSENACS…` |
| ⚠️ **事前審査** | `01JSE0CREC…` ＋ ⚠️ **`01J95TGVT…`（現金確認）** |
| 申込 | `01J82Z5F1RR18…` |
| ⚠️ **契約** | `01JP74NGRT…` ＋ ⚠️ **`01JV6AVXQ…`（仲介）** |

⚠️ ⚠️ **複数列は「どれかに日付があれば到達」**（画面の `getOldestDate` と同じ考え方）。

### 触ったファイル

| ファイル | 変更 |
|---|---|
| `columns.ts` | ⚠️ **`DIVISION_CONFIG` / `PHASE_COLUMNS` / `PHASE_ORDER` / `phaseReached` / `phaseDateOf`** |
| `metrics.ts` | ⚠️ **`metricSqlFor` / `medianValueSqlFor`**、⚠️ **`contacts` と `applications` を追加** |
| `query.ts` | ⚠️ テーブルと基準日を事業で切り替え |
| `index.ts` | ⚠️ **`division` パラメータ**（pivot / funnel） |
| `meta.ts` | ⚠️ **建売の工程と失注の扱いを meta に明記** |
| `mcp-server/src/index.ts` | ⚠️ 各ツールに `division` |

⚠️ ⚠️ **既定は `order`。** ⚠️ **指定しなければ今までと同じ結果**になる。

### ⚠️ 途中で見つけた不具合

⚠️⚠️ **注文の `applications` が契約と同じ件数になっていた**（534件）。
⚠️ ⚠️ **「その事業に無い工程」を丸めると、上位工程だけで数えてしまう。**
⚠️ `reachedFrom()` の先頭で ⚠️ **列が無ければ 0 を返す**ようにして直した。

### 確認（2026-09-22・ローカル）

⚠️ ⚠️ **SQL で直接数えた値と API が一致した。**

| | SQL | API |
|---|---|---|
| leads | 7,651 | ⚠️ **7,651** |
| contracts | 578 | ⚠️ **578** |
| applications | 764 | ⚠️ **764** |

⚠️ `visits` は SQL 2,349 / API 2,360。⚠️ ⚠️ **照合SQL側が「現金確認」を漏らしていたため**で、
⚠️ **API は画面（`screeningKeys` に現金確認を含む）と同じ。**

⚠️ 実測（年別）

| 年 | leads | contacts | visits | 次アポ | 申込 | 契約 |
|---|---|---|---|---|---|---|
| 2025 | 2,780 | 1,920 | 912 | 358 | 260 | 258 |
| 2026 | 2,728 | 1,563 | 787 | 324 | 216 | 200 |

⚠️ ⚠️ **`lost` は建売では null**（⚠️ 0件と区別できるようにしてある）。

---

## ⚠️ 11. 中古リノベの粗利額が崩れる（原因は浮動小数点）

⚠️ 利用者:

> rank/RankResale.tsx について
> このコンポネントは粗利額を表示するので表示がくずれる
> スタイルはこのままでよいので表示方法は前に戻す

⚠️ ⚠️ **まず表示方法を改修前へ戻した**（`aa2d1312`）が、⚠️ **本番では直らなかった。**
...
⚠️⚠️ **CSS ではなく、足し算の誤差だった。**
⚠️ ⚠️ **`budget_total` は万円単位の小数**で、⚠️ **そのまま `reduce` で足すと二進小数の誤差が出る。**
⚠️ ⚠️ **その桁が列幅を押し広げて崩れていた。**

⚠️ ⚠️ **改修とは無関係で、元からの不具合である。**
...
⚠️ 同じ書き方は ⚠️ **他の画面には無い**ことを確認した（`budget_total` を使うのはこの画面だけ）。

### ⚠️ 切り分けでやったこと

⚠️ ⚠️ **本番が古いファイルを配っている可能性を先に潰した。**

| 確認 | 結果 |
|---|---|
| `RankResale.tsx` と production の差分 | ⚠️ **ゼロ**（戻せていた） |
| 本番の `index.html` が指す JS | ⚠️ **新しい方**（`main.c00289b2.js`） |
| ⚠️ **配信中の JS とローカルのビルド** | ⚠️ **MD5 一致**（⚠️ **配信は正しかった**） |

⚠️ ⚠️ **ここまでで「配信の問題ではない」と分かり、画面の値を見て原因にたどり着いた。**
⚠️ ⚠️ **スクリーンショット1枚で確定した。** ⚠️ **崩れの report には実際の表示を添えてもらうのが早い。**
