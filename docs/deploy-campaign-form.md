...
`frontend/build/` を ① の公開ディレクトリへ。⚠️ `index.html` の上書き漏れに注意。

⚠️ 画面は `campaign/` にまとまり、タブの一番右に**フォーム作成**が増える。
⚠️ 旧API（`khg-marketing.info/api/`）は叩かなくなる。
⚠️⚠️ これにより**ダッシュボードの認証を通るようになる**。
　 ⚠️ 旧経路は `Authorization` をルーティングに使っており Token を送っていなかったため、
...
| 11 | ⚠️ khg の `mail_cc` に **`※ここは編集しない`** というプレースホルダが入っており、`Cc:` が壊れたヘッダになっていた（14フォーム）。無害化で落ちる |

---

## ⚠️ 退避したときの通知について

⚠️ ② が落ちて ① へ退避したときは、① が**リクエストの宛先しか見ない**。

| | ② が生きている | ⚠️ ① へ退避 |
|---|---|---|
| 通知先 | `form_table` から引く（**ブラウザに出ない**） | ⚠️ ① の `form_get` 応答に含まれる値を使う |
| khg の店舗別 Cc | ⚠️ ② の `KHG_CC` が効く | ⚠️ **効かない**（`form_table` の値になる） |
| khg のブランド | ② が解決 | ⚠️ フロントが解決して送る（`KHG_BRAND`） |

⚠️⚠️ **`Form.tsx` で `mail_to` / `mail_cc` を空に固定しないこと。**
⚠️ 固定すると、退避したときに**社内通知が1通も飛ばなくなる**（しかも気づけない）。
⚠️ 一度その状態を作ってしまい、検証で見つけて直した。

⚠️ khg の店舗別 Cc は `form_table` に入っていない（プレースホルダのまま）ため、
⚠️ **退避時は `mail_to` にしか届かない。**
⚠️ 恒久的に直すなら `form_table.mail_cc` に実際の宛先を入れること。

### ⚠️ 確認済み（ビルド成果物に対して）

⚠️ ソースではなく**実際に配布されるバンドル**を検査している。

- ⚠️ 社内の通知先14件が**1件も含まれない**（総当たりでメールアドレス0件）
- ⚠️ ② を叩き、**退避先（①）も残っている**
- ⚠️ khg の店舗対応（8件）が入っている
- ⚠️ 応答の `mail_to` / `mail_cc` を**受け取っている**（空で固定していない）
