...
| 8 | ⚠️ **勝敗分析を担当者・店舗で絞れるように** | ② Express / 利用者のPC |
| 9 | ⚠️⚠️ **分析APIを建売分譲事業に対応**（`division=kaeru`） | ② Express / 利用者のPC |
...
⚠️ ⚠️ **ビルド済み**（2026-09-22 19:34）。

| ファイル | 確認したこと |
|---|---|
| ⚠️ **`build/static/js/main.c00289b2.js`** | ⚠️ **`2.2.145` を含む。RankResale を戻したあとのビルド** |
| `build/static/css/main.7c10f266.css` | ⚠️ **v2.2.144 から変わっていない**（⚠️ それでも一緒に上げてよい） |
| `build/index.html` | ⚠️ **上の2つを参照している** |

⚠️⚠️ **`main.ee94e56a.js`（18:14）は使わないこと。**
⚠️ ⚠️ **RankResale（中古リノベの目標達成状況）が崩れた版である。**
⚠️ 2026-09-22、⚠️ **直したあとにビルドし直さず、古い版を上げて本番で崩れた。**
...
| 4 | ⚠️ **「KH八代店の井上健太郎の勝敗分析をHTMLで」** | ⚠️ **その人の商談だけで返る**（⚠️ 実測15件） |
| 5 | ⚠️⚠️ **「建売分譲事業の歩留まりを教えて」** | ⚠️ **接触・来場・申込・契約で返る**（⚠️ `division=kaeru`） |
