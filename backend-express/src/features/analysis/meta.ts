...
export const caveats = (): string[] => [
  ...
  '⚠️ 上の入力率の問題に対応するため、ダッシュボードのKPI画面（店舗別動向など）は' +
    '「上位の工程に進んだ人は、下位の工程も達成したもの」として数えている。' +
    '契約日は必ず入るが初回面談日が空のまま契約済みの顧客が実在し、素直に数えると' +
    '契約数が面談数を上回る逆転が起きるためである。' +
    '⚠️ これは営業担当が前の工程の日付を入力しないという運用上の癖への対処であって、' +
    '実際に工程を飛ばしたわけではない。' +
  ...
  '⚠️⚠️ 担当者（staff 軸）は master_data.in_charge_user をそのままは使っていない。' +
    '失注や案件の長期化が起きると担当が「◯◯店 管理」に付け替えられる運用のため、' +
    'その場合は first_interviewed_user（旧担当）を担当として扱っている。' +
    '⚠️ 実測では 24,607件中 17,822件（72%）が「◯◯店 管理」になっており、' +
    'そのうち 4,700件は旧担当が入っているので営業名に戻せる。' +
    '⚠️ 旧担当も空なら「◯◯店 管理」のまま出る（誰の実績か分からない顧客）。' +
    '⚠️ 付け替え前の生の値を見たいときは staffCurrent 軸を使うこと。',
  '⚠️⚠️ first_interviewed_user（旧担当）の記録を運用として始めたのは2026年6月である（担当者の説明）。' +
    'それより前の顧客は、担当が「◯◯店 管理」に変わっていても旧担当に戻せないことがある。' +
    '⚠️ つまり2026年5月以前ほど「◯◯店 管理」のまま残る顧客が多く、' +
    '古い月ほど営業別の実績が実態より小さく出る。' +
    '⚠️ 月をまたいで営業別の件数を比較するときは、この差を必ず断ったうえで述べること。' +
    '⚠️ 参考までに列が埋まっている割合は 2025年 86%（13,125件中11,340件）、' +
    '2026年 51%（8,781件中4,450件）で、値自体は2026年6月より前にも入っている。' +
    '⚠️ 埋まっていること自体は、その値が運用として維持されていたことを意味しない。',
  '⚠️ 担当者は「その顧客をいま担当している人」である。' +
    '担当替えがあると過去の実績ごと新しい担当者に移るため、' +
    '過去の月の数字が後から変わることがある。' +
    '⚠️ 反響を取った人でも、面談をした人でもない。' +
    '⚠️ 面談を実際に誰が行ったかは interviewsLed（面談シートの staff との一致）で見るが、' +
    'staff が記録されている面談ログは全体の1割ほどしかないため下限値である。',
