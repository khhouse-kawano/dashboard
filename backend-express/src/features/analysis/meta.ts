...
export const caveats = (): string[] => [
  ...
  '⚠️ 上の入力率の問題に対応するため、ダッシュボードのKPI画面（店舗別動向など）は' +
    ...
    '実際に工程を飛ばしたわけではない。' +
    '⚠️ その数え方に合わせた指標が visits（実来場数）/ nextAppointments（次アポ数）/ ' +
    'reservations（来場予約数）である。' +
    '⚠️ 画面の数字と突き合わせるときは firstInterview / secondInterview ではなく' +
    'visits / nextAppointments を使うこと。両者は必ず食い違う。',
