...
const Edit = ({ editId, setEditId }: Props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post(API_BASE, { request: 'k-snap_edit' }, { headers: API_HEADERS });
    ...
    const handleShow = async(id: string, flag: number) => {
        if (!id) return;
        try {

        } catch (err) {
            alert('通信に失敗');
            setList(prev => prev.map(v => v.id === id ? { ...v, show_snap: flag ? 0 : 1 } : v));
        }
    };
                ...
                <Table responsive borderless className="align-middle">
                    ...
                    {handleSlice(list, activePage).map((item) => (
                        <tbody key={item.id} className="border-bottom" style={{ transition: 'background-color 0.2s' }}>
                            <tr style={{ opacity: item.show_snap === 1 ? 1 : 0.55 }}>
                                            ...
                                            {item.image ? (
                                                ...
                                                >
                                                    <img
                                                        ...
                                                        src={`${IMAGE_BASE_URL}/${item.image.replace(/^\//, "")}`}
                                                        alt="snap"
                                                    />
                                                </div>
                                            ) : (
